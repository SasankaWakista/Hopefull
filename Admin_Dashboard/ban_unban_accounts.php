...
if (isset($_POST['action']) && isset($_POST['id'])) {
    ...
    $sql_check_role = "SELECT role FROM admin_users WHERE id='$id'";
    $result_check_role = $conn->query($sql_check_role);

    if ($result_check_role && $result_check_role->num_rows > 0) {
        $row_check_role = $result_check_role->fetch_assoc();

        // Only proceed if the role is not 'System Admin'
        if ($row_check_role['role'] !== 'System Admin') {
            if ($_POST['action'] == 'ban') {
                $sql = "UPDATE admin_users SET status='banned' WHERE id='$id'";
            } elseif ($_POST['action'] == 'unban') {
                $sql = "UPDATE admin_users SET status='active' WHERE id='$id'";
            }
            $conn->query($sql);
        }
    }
...
$sql_banned = "SELECT id, name, role, email FROM admin_users WHERE status='banned'";
$sql_active = "SELECT id, name, role, email FROM admin_users WHERE status='active'";
...
<head>
    ...
    <title>Ban/Unban Admin Accounts</title>
